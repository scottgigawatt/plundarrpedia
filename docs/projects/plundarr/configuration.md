...
Use its numeric values for `DEFAULT_PUID` and `DEFAULT_PGID`. Set
`DEFAULT_GROUP` to the supplemental group that owns shared media or
configuration paths, then confirm that downloads, final libraries, and
`config/` are writable where appropriate.

Plundarr applies these settings in two deliberate ways:

- Apprise and Seerr run directly as `DEFAULT_PUID:DEFAULT_PGID` with
  `DEFAULT_GROUP` added for shared host-path access.
...
This rootless behavior is generated by Maraudarr v1.0.2 and newer. Refresh
Maraudarr and regenerate an older Compose file before troubleshooting its
runtime identity.
